package com.bceden.current.condition;
	    ...
	    public static void main(String[] args) throws Exception{
	        ...
	        Thread thread2 = new Thread("Thread 2 "){
	            ...
	            public void run() {
	                ...
	                logger.info(Thread.currentThread().getName() + " 正在运行.....");
	                logger.info(Thread.currentThread().getName() + " 开始中断线程1.....");
	                thread1.interrupt(); // 对线程1 进行中断 看看中断后会怎么样? 结果 线程 1还是获取lock, 并且最后还进行 lock.unlock()操作

	                try {
	                	logger.info(Thread.currentThread().getName() + " 开始睡眠.....");
	                    Thread.sleep(2 * 1000);
	                }catch (Exception e){

	                }
	                logger.info(Thread.currentThread().getName() + " 发送唤醒信号.....");
//	                condition.signal(); // 发送唤醒信号 从 AQS 的 Condition Queue 里面转移 Node 到 Sync Queue
	                logger.info(Thread.currentThread().getName() + " 已发送一个 signal ");
